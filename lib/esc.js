...
module.exports = {
  ...
  checkFaultyValue: function(mElement, lValue){
    // set value range
    if(lValue < 1000)
      lValue = 1000;
    else if(lValue > 2000)
  ...
  setAllESC: function(lValue){
    console.log("[ESC] setAllESC lValue:" + lValue);
  ...
  setMultipleESC: function(aESC, lValue){
    if(this.checkFaultyValue(aESC, lValue))
      return;
    var sValue = "";
    for(var lIndex in aESC){
  ...
  setESC: function(lPin, lValue){
    if(this.checkFaultyValue(lPin, lValue))
      return;
